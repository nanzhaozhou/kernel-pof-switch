...
static uint8_t poflr_mm_tbl_num = POFLR_MM_TBL_NUM;
static uint8_t poflr_lpm_tbl_num = POFLR_LPM_TBL_NUM;
static uint8_t poflr_em_tbl_num = POFLR_EM_TBL_NUM;
static uint8_t poflr_dt_tbl_num = POFLR_DT_TBL_NUM;

/* Flow table size. */
static uint32_t poflr_flow_table_size = POFLR_FLOW_TABLE_SIZE;

/* Flow tables. Defined in pof_local_resource.h-->. */
static poflr_flow_table *poflr_table_ptr[POF_MAX_TABLE_TYPE];

/* Key length. */
static uint32_t poflr_key_len = POFLR_KEY_LEN;

/* Key length of each type flow table. */
static uint16_t poflr_key_len_each_type[POF_MAX_TABLE_TYPE];

/* Table number of each type flow table. */
static uint8_t poflr_table_num_each_type[POF_MAX_TABLE_TYPE];

/* Table ID's base value of all types. */
static uint8_t poflr_key_tid_base_each_type[POF_MAX_TABLE_TYPE];
...
uint32_t poflr_init_flow_table(void){
    uint32_t i;

    /* Initialize the key length of each table type. */
	/* Enum defined in pof_global.h-->. */
    ...
    for(i=0; i<POF_MAX_TABLE_TYPE; i++){
        poflr_table_ptr[i] = (poflr_flow_table*)kmalloc(poflr_table_num_each_type[i] * sizeof(poflr_flow_table), GFP_KERNEL);
		if(poflr_table_ptr[i] == NULL){
			/* Defined in pof_local_resource.c-->. */
...
uint32_t poflr_free_flow_table(void){
	uint32_t i, j;

	for(i=0; i<POF_MAX_TABLE_TYPE; i++){
		if(NULL != poflr_table_ptr[i]){
			for(j=0; j<poflr_table_num_each_type[i]; j++){
				kfree(poflr_table_ptr[j][i].entry_ptr);
			}
		}
		kfree(poflr_table_ptr[i]);
	}
	return POF_OK;
}

/* Empty flow table. */
uint32_t poflr_empty_flow_table(void){
								 ...
								 pof_match *match)
{
    ...
    tmp_tbl_ptr->entry_ptr = (poflr_flow_entry *)kmalloc(size * sizeof(poflr_flow_entry), GFP_KERNEL);
    POF_KMALLOC_ERROR_HANDLE_RETURN_UPWARD(tmp_tbl_ptr->entry_ptr, g_recv_xid);
    ...
    printk(KERN_DEBUG "[POF_DEBUG_INFO:] Create flow table SUC!\n");
    return POF_OK;
}
...
uint32_t poflr_delete_flow_table(uint8_t table_id, uint8_t type){
    ...
    printk(KERN_DEBUG "[POF_DEBUG_INFO:] Delete flow table SUC!\n");
    return POF_OK;
}
...
uint32_t poflr_add_flow_entry(pof_flow_entry *flow_ptr){
    ...
    printk(KERN_DEBUG "[POF_DEBUG_INFO:] Add flow entry SUC! Totally %d entries in this table.\n",
            tmp_tbl_ptr->entry_num);
...
uint32_t poflr_delete_flow_entry(pof_flow_entry *flow_ptr){
    ...
    printk(KERN_DEBUG "[POF_DEBUG_INFO:] Delete flow entry SUC!\n");
    return POF_OK;
}
...
uint32_t poflr_modify_flow_entry(pof_flow_entry *flow_ptr){
    ...
    printk(KERN_DEBUG "[POF_DEBUG_INFO:] Modify flow entry SUC!\n");
    return POF_OK;
}
...
uint32_t poflr_check_flow_table_exist(uint8_t ID){
    ...
    ret = poflr_table_ID_to_id(ID, &table_type, &table_id);
	if(POF_OK != ret){
		return POF_ERROR;
	}
