 ...
 *           direct table means linear table in controller.
 * NOTE:     If there is an ERROR, The packet_over identifier will be TRUE.
 ***********************************************************************/
...
static uint32_t execute_GOTO_TABLE(POFDP_ARG)
{
    ...
    key_ptr = (uint8_t **)kmalloc(POF_MAX_MATCH_FIELD_NUM * sizeof(uint8_t *), GFP_ATOMIC);
    POF_KMALLOC_ERROR_HANDLE_RETURN_NO_UPWARD(key_ptr);

    for(i=0; i<POF_MAX_MATCH_FIELD_NUM;  i++){
        key_ptr[i] = (uint8_t *)kmalloc(POF_MAX_FIELD_LENGTH_IN_BYTE, GFP_ATOMIC);
        if(key_ptr[i] == NULL){
            for(j=0; j<i; j++){
                kfree(key_ptr[j]);
            }
