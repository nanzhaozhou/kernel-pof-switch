...
/* Define the max string length of the interface name. */
...
/* if return value is illegal then terminate the switch. */
...
#define POF_KMALLOC_ERROR_HANDLE_RETURN_NO_UPWARD(ptr) \
    if((ptr) == NULL){ \
...
#define POF_KMALLOC_ERROR_HANDLE_RETURN_UPWARD(ptr,xid) \
    if((ptr) == NULL){ \
...
extern void terminate_handler(void);
